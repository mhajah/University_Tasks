...
from execute_api import api_get_movies
from execute_api import api_add_movie
from execute_api import api_delete_movie
from execute_api import api_update_movie
...
if __name__ == "__main__":
    ...
    parser.add_argument('action', choices=['add', 'list', 'bind', 'delete', 'update'], help='Action to perform')
    parser.add_argument('api', choices=['api', 'direct'])
    ...
    if args.api == 'direct':
        ...
        if args.entity == 'movies':
            if args.action == 'add':
                api_add_movie(args.title, args.director_id, args.release_year, args.movie_length)
                
            elif args.action == 'list':
                movies = api_get_movies()
                print(movies)
                for movie in movies:
                    print(f"({movie[0]}) Title: {movie[1]}, \n Release year: {movie[3]} \n Length: {movie[4]} min.")

            elif args.action == 'update':
                api_update_movie(args.movie_id, args.title)

            elif args.action == 'delete':
                api_delete_movie(args.movie_id)
